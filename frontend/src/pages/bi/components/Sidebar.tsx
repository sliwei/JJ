...
const Sidebar: React.FC<SidebarProps> = ({ ups, activeMid, unreadCounts, onSelectUP, onOpenSettings, onAddUP, theme, onToggleTheme }) => {
  return (
    <aside className="w-60 bg-sidebar border-r border-border flex flex-col transition-all duration-300">
      ...
      <div className="flex-1 overflow-y-auto p-2">
        {ups.map((up) => (
          <div
            key={up.mid}
            className={`flex items-center justify-between p-2 mb-1.5 rounded-lg cursor-pointer transition-colors duration-200 hover:bg-hover ${activeMid === up.mid ? 'bg-primary/10 border border-primary/40' : 'border border-transparent'}`}
